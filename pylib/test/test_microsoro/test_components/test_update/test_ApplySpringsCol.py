...
def test_py_stretched_mixed_vy(event_buffer: typing.Optional[EventBuffer]):
    state = State()
    ...
    assert np.all(state.vx == 0)

    assert all_cols_equivalent(state.vy)
    assert np.all(state.vy[0, :] > 0)
    assert np.all(state.vy[-1, :] < 0)


def test_py_compressed_mixed_vy(event_buffer: typing.Optional[EventBuffer]):
    state = State()
...
def test_py_stretched_rotation_invariants(
    ...
    for rotate_degrees in range(360):
        ...
        res = ApplySpringsCol()(state, event_buffer)
        assert res is None
